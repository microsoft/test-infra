        ...
        stage("ACC-1804 Generate Quote") {
            ...
            steps{
                script{
                    ...
                    def task = """
                            cmake ${WORKSPACE}/openenclave -G Ninja -DHAS_QUOTE_PROVIDER=ON -DCMAKE_params.BUILD_TYPE=${params.BUILD_TYPE} -Wdev
                            ninja -v
                            ...
                            popd
                            """
                    runner.ContainerRunLegacy("openenclave/ubuntu-1804:latest", "clang-7", task, "--cap-add=SYS_PTRACE --device /dev/sgx:/dev/sgx")
                    ...
                    stash includes: 'build/tests/host_verify/host/*.der,build/tests/host_verify/host/*.bin', name: "linux_host_verify-${params.LINUX_VERSION}-${params.BUILD_TYPE}-${BUILD_NUMBER}"
                }
            }
        }
        ...
        stage("Linux nonSGX Verify Quote") {
            ...
            steps{
                script{
                    ...
                    def runner = load pwd() + "${SHARED_LIBRARY}"
                    runner.checkout("${params.PULL_NUMBER}")
                    unstash "linux_host_verify-${params.LINUX_VERSION}-${params.BUILD_TYPE}-${BUILD_NUMBER}"
                    def task = """
                            cmake ${WORKSPACE}/openenclave -G Ninja -DBUILD_ENCLAVES=OFF -DHAS_QUOTE_PROVIDER=OFF -DCMAKE_params.BUILD_TYPE=${params.BUILD_TYPE} -Wdev
                            ninja -v
                            ctest -R host_verify --output-on-failure --timeout 1200
                            """
                ...
                script{
                    ...
                    //docker.image('openenclave/windows-2019:latest').inside('-it --device="class/17eaf82e-e167-4763-b569-5b8273cef6e1"') { c ->
                        ...
                        dir('build') {
                            bat """
                                vcvars64.bat x64 && \
                                cmake.exe ${WORKSPACE}/openenclave -G Ninja -DBUILD_ENCLAVES=OFF -DHAS_QUOTE_PROVIDER=OFF -DCMAKE_params.BUILD_TYPE=${params.BUILD_TYPE} -DNUGET_PACKAGE_PATH=C:/oe_prereqs -Wdev && \
                                ninja -v && \
                                ctest.exe -V -C ${params.BUILD_TYPE} -R host_verify --output-on-failure --timeout 1200
                                """
                        }
                    //}
                }
